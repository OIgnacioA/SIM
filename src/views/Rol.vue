<template>
    <div class="page">
        ...
        <!-- Titulo -->
        <div class="txt_title">¿Cual es tu rol informático?</div>
        <!-- Subtitulo -->
        <div class="txt_subtitle">Comprende las tareas específicas a realizar en un puesto determinado.</div>
        <!-- Input -->
        <Dropdown v-model="selectedRol" :options="simStore.roles" optionLabel="nombre" placeholder="Seleccionar"
            class="input" />
        <!-- Descripcion -->
        <div class="dsc" v-if="selectedRol">
            {{ selectedRol.descripcion }}
        </div>
    </div>
    <!-- Botonera -->
    <div class="footer">
        <div class="next" :class="{ next_enabled: selectedRol }"
            @click="selectedRol ? router.push({ path: '/experiencia' }) : void (0)">
            SIGUIENTE
        </div>
...
selectedRol.value = simStore.rolSeleccionado ? simStore.rolSeleccionado : null

watch(selectedRol, (newRol) => {
    simStore.selectRol(newRol)
})
